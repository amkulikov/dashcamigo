...
|---|---|---|
| Novatek freeGPS Type-3 (float DDmm) | 2E Drive 730, SilverStone F1 A80, VIOFO A119, A329, A329S, T130 (seq-less `_PR`/`_PF` parking names), A119 Mini Pro / Mini 2 (14+6-digit names) | datetime@44 default layout. Newer VIOFO firmwares (A119 Mini Pro/Mini 2) use the 14-digit+counter naming, not the classic `YYYY_MMDD_...` shape |
...
| 70mai embedded freeGPS (int32×1e7) | 70mai A810, M500 | int32×1e7 decimal-degree coords (NOT Novatek float DDmm), heading@+35, **no per-record timestamp** (only a constant 8h-shifted file-start unix) -> records emitted `timeUnsynced` and re-anchored onto the video window; speed reconstructed from the trajectory (the block's candidate speed field has no clean unit), GPS-gap spikes capped to 0 rather than charted as 1000+ km/h. Verified A810/M500 (2999- and 1800-record blocks; tracks decode to realistic speed). Layout: `freegps-70mai.ts`. A510 confirmed (incl. its `LA`/`PA` parking-mode filename prefixes; rear `B` clips carry no GPS - front only). Likely also Omni/X200 |
...
- `gps0` is NOT "Ambarella/Novatek" per ExifTool - that string appears nowhere
  ...
  name.
- 70mai embedded coordinates are `int32 × 1e7`, not `× 1e5`.
