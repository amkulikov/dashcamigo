...
| 70mai embedded freeGPS (ddmm×1e5) | 70mai A810, M500 | int32×1e5 NMEA ddmm.mmmm coords (NOT decimal degrees; ExifTool's A810 branch - QuickTimeStream.pl GPSType 19 - reads the same offsets as /1e5 + minutes conversion), heading@+35, speed field is km/h but unused (trajectory-derived speed kept; standstill/gap behavior unverified), **no per-record timestamp** (only a constant 8h-shifted file-start unix) -> records emitted `timeUnsynced` and re-anchored onto the video window; speed reconstructed from the trajectory, GPS-gap spikes capped to 0 rather than charted as 1000+ km/h. Coordinates verified against the on-video OSD stamp (digit-exact) and ExifTool's independent decode. Layout: `freegps-70mai.ts`. A510 confirmed (incl. its `LA`/`PA` parking-mode filename prefixes; rear `B` clips carry no GPS - front only). Likely also Omni/X200 |
...
| `gps0`+IDIT tail atoms | Navitel R600-1, iBOX iCON | ExifTool Process_gps0: speed u16 km/h @20, altitude i32 @16, in-record year-2000/month @22-23, course/2 @28. Firmware interleaves ~1/3 stale ring-buffer rows (older timestamps, already-driven positions) -> dropped in-parser (`navitel-gps0.ts`). Shared parser: ExifTool also attributes this family to DuDuBell M1 / VSYS M6L / Lamax S9 (likely covered, unverified) - Auto-Vox reuses the `gps0`/`gsen` NAMES but in a RIFF trailer with different record layouts, and has its own primitive. iBOX is multi-SoC: the **iCON** is Ambarella/gps0 (this row); the **RoadScan 4K** is SigmaStar player-only GPS, nothing in the file to parse |
...
| LigoGPS trailer on MPEG-TS (plaintext table) | unknown-vendor 2-channel `<14-digit>_<7-digit><F\|R>.ts` | the same firmware family writes BOTH carriers: encrypted LigoGPS on its own private PES (unclaimed, the fuzz question of the trailer twin) and a plaintext table appended past the last whole 188-byte packet. Magic is rebranded `SKIPLCAIGPSINFO`; slots are 132 bytes (u32 index + ASCII record), speed km/h, `A:` course, no accel; timestamps camera-local, strict 1 Hz. Front and rear carry the SAME table byte-for-byte in the slots. The off-grid trailer breaks every mediabunny TS scan (whole cards read as "empty folder"), so AV consumers clamp reads to the clean prefix - detection + clamp: `src/ts-trailer.ts`; parse: `internal/ligogps.ts` (parseLigoGpsTsTrailer) |
| SigmaStar 40-byte `ssmd` GPS track | Neoline Spectrum | dedicated 1 Hz ssmd meta track (NOT the LigoGPS ssmd dialect, NOT Rove's 32-byte one): f64 decimal-degree coords with a 4294967295.0 no-fix sentinel, u16 speed raw km/h, course/2 byte (same halving as gps0), day/hour/min/sec only - **UTC on fix rows, camera-local RTC on no-fix rows**; year/month anchored to the filename date. Sibling tracks: JPEG thumbnail + a 12-byte ~15 Hz telemetry track (unparsed - undecoded axes). Layout: `docs/format-sstar-ssmd.md` |
