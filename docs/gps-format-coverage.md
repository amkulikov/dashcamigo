...
| LigoGPS trailer on MPEG-TS (plaintext table) | See [format notes](format-ligogps-trailer-ts.md) | Detection and AV clamp: `src/ts-trailer.ts`; telemetry: `parseLigoGpsTsTrailer` in `src/parsers/internal/ligogps.ts` |
| SigmaStar 40/56-byte `ssmd` GPS track | Neoline Spectrum; iZEEKER iD300 | dedicated ~1 Hz ssmd meta track (NOT LigoGPS, NOT Rove's 32-byte dialect). Neoline 40-byte rows carry direct f64 degrees and a no-fix sentinel; iZEEKER 56-byte KTRX rows carry reversible encoded coordinates plus a scrubbed stable identifier. Shared fields: integer km/h speed, course/2, day/hour/min/sec; year/month anchored to the filename date. Layout and validation: `docs/format-sstar-ssmd.md` |
...
| Novatek GPS struct in MPEG-TS PES (bare) | VIOFO A119 V3; Avylet A1 with the optional GPS receiver; TS recording mode (`<14-digit>_<6-digit>.TS`) | classic freeGPS record struct (HH MM SS YY MM DD i32 + A/NS/EW + f32 DDmm + knots + course) as the ENTIRE payload of a 1008-byte private_stream_2 PES, PID auto-detected (NOT in the PMT), 1 record/s; no-fix Avylet rows use `V00` with unset clock fields; struct clock is camera-LOCAL -> `timeUnsynced`. The B4K prefix dialect has its own row in the foreign-source table. Layout: `docs/format-novatek-ts.md` |
...
| NMEA `.nmea` sidecar | samplefiles MOV_0581, Navman MiVue 150 Safety, Mio MiVue 955WD Pro | The 955WD Pro writes synchronized `FILE<YYMMDD>-<HHMMSS>F/R.mp4` clips into sibling F/R folders; the front clip owns the same-basename `.NMEA` sidecar. The shared iBox-shaped filename techniques provide time/channel metadata, while the camera key strips both folder letters so the pair stays one frame. |
